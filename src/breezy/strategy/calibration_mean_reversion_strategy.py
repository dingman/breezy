"""Calibration Mean-Reversion Strategy — standalone NautilusTrader module

Trades statistically large gaps between market implied probability and calibrated NWS probabilities.
...
from calibration_mean_reversion_strategy import (
...
strategy = CalibrationMeanReversionStrategy(
    CalibrationMeanReversionConfig(instrument_ids=(contract.instrument_id,)),
    registry,
    WeatherProbabilityEngine(),
)
...
class ForecastErrorRecord:
    ...
    ``published_at`` must be the original issuance time. ``realized_high_f``
    is the official climate-observation high, known only after settlement.
    Calibration tables are built offline from these records; they are NOT
    updated inside the backtest from future realizations.
    """
...
def fit_error_model(
    ...
    """Offline calibration. Call this on a train window that ends before the
    backtest start date to avoid lookahead.
    """
    model = base or ForecastErrorModel(min_samples_for_local=min_samples)
    buckets: dict[str, list[float]] = {}
    helper = WeatherProbabilityEngine(model)
    for rec in records:
    ...
    _ = helper  # kept to document that the same engine consumes this model
    return model


class CalibrationMeanReversionConfig(WeatherStrategyConfig):  # type: ignore[misc]
    strategy_id: str | None = "CALIBRATION-MR-001"
    entry_z: float = 1.85
    exit_z: float = 0.60
...
class CalibrationMeanReversionStrategy(WeatherStrategyBase):
    def __init__(
        self,
        ...
        self.config: CalibrationMeanReversionConfig = config
    ...
    def evaluate_instrument(
        ...
        cfg = self.config
        last = self._last_eval.get(contract.instrument_id)
        if last is not None:
            dt_min = (now - last).total_seconds() / 60.0
            if dt_min < cfg.recheck_minutes and abs(self.portfolio_view.net_qty(contract.instrument_id)) < 1e-9:
                return None
        self._last_eval[contract.instrument_id] = now

        hours_left = self.hours_to_settlement(contract, now)
        ...
        cal_p = self.engine.contract_probability(
            contract, forecast.expected_high_f, forecast.horizon_hours
        )
        bid_p, ask_p, mid_p = self.implied_probs(contract, quote)
        if mid_p is None or bid_p is None or ask_p is None:
            return None

        se = self.engine.expected_probability_se(
            cal_p, forecast.horizon_hours, extra_market_noise=cfg.extra_market_noise
        )
        z_mid = (mid_p - cal_p) / se
        gap = mid_p - cal_p

        current = self.portfolio_view.net_qty(contract.instrument_id)
        if current > 1e-9 and z_mid > -cfg.exit_z:
            # we are long YES because market was too cheap (z negative)
            return SignalDecision(
                contract.instrument_id,
                SideIntent.FLAT,
                cal_p,
        ...
        qty = min(cfg.max_quantity, cfg.base_quantity + cfg.z_qty_scale * (abs(z_mid) - cfg.entry_z + 1.0))
        return SignalDecision(
