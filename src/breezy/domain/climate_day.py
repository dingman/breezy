...
Lives in `domain` (the bottom layer), not `normalize`: `breezy.domain`
records (e.g. `StationObservation.climate_day`) need this conversion, and
the layer contract in `pyproject.toml` forbids `domain -> normalize`.
`breezy.normalize.climate_day` re-exports these names unchanged for every
pre-existing caller above `domain` in the layer stack.
"""
