...
| 2026-09-02 | retail read surface enumeration (SDK 0.1.2 + `docs.polymarket.us` llms.txt) | n/a | **NO retail price-history / trades-history / candlestick endpoint exists.** The complete Stack-A market-data surface is `GET /v1/markets`, `/v1/market/id/{id}`, `/v1/market/slug/{slug}`, `/v1/markets/{slug}/book`, `/v1/markets/{slug}/bbo`, `/v1/markets/{slug}/settlement` — six paths, all point-in-time. Candlestick and historical market data appear ONLY under the institutional `data-guide/*`, `trader-guide/reporting`, and `api-reference/report/*` doc sections (Polymarket **Exchange**, Private Key JWT, gRPC/FIX) — Stack B, unreachable with retail Ed25519 credentials. 15 speculative slug-scoped GET paths (`history`, `prices-history`, `price-history`, `trades`, `candles`, `candlesticks`, `ohlc`, `timeseries`, `stats`, `chart`, `prices`, `ticks`, `quotes`, …) all returned `404`/code `5`. | true | Reproduced on 2 distinct expired weather markets (`tc-temp-laxhigh-2026-08-25-gte81lt82f`, `tc-temp-nychigh-2026-08-27-gte84lt85f`); doc-surface check independent. |
| 2026-09-02 | `GET gateway /v1/price-history` | `tc-temp-nychigh-2026-08-27-gte84lt85f` | **An UNDOCUMENTED `/v1/price-history` route EXISTS on the public gateway and is absent from SDK 0.1.2.** It returns `400` / grpc `code` 3 (INVALID_ARGUMENT) on every GET, while three near-miss controls (`/v1/price-histor`, `/v1/price-historyy`, `/v1/zzz-not-a-path`) return `404` / `code` 5 — so the route matches and only the arguments are rejected. The same path **authenticated** against `api.polymarket.us` returns `404` / `code` 5: it is gateway-only. 40+ query-parameter shapes were swept (identifier keys `marketSlug|slug|market|symbol|marketId|marketIds|slugs|symbols|instruments|eventSlug` incl. snake_case; time pairs `startTime/endTime`, `startTs/endTs`, `start/end`, `from/to`, `startDate/endDate`; interval keys `interval|period|resolution|granularity|fidelity|candleInterval`; plus `limit/offset`, `side`, `outcome`, path-param forms `/v1/price-history/{slug}` and `/{id}`) — **every one returned `400`/3**, and no parameter changed the code. `message` is the same generic string the venue returns on all grpc codes, so it carries zero discriminating information. | true | Reproduced across ~45 requests, 1 slug + 1 numeric market id + 3 negative controls, one session. Needs a second slug before promotion. |
| 2026-09-02 | `GET gateway /v1/markets/{slug}/book` on **settled** markets | 180 climate markets, 2026-08-25..2026-08-31 (5 cities × 6 rungs × 6 station-days) + 90 markets 2026-06-10..12 | **Settled weather markets stay fully queryable and their `/book` carries a retained per-market daily TRADE SUMMARY.** `marketData.state` = `MARKET_STATE_EXPIRED`, `bids` = `[]` and `offers` = `[]` (book is emptied), but `marketData.stats` gains a terminal key-set: `openPx, openSetTime, closePx, closeSetTime, highPx, highSetTime, lowPx, lowSetTime, lastTradePx, lastTradeQty, lastTradeSetTime, sharesTraded, notionalTraded, notionalSetTime, currentPx, settlementPx, settlementSetTime, openInterestSetTime, settlementPriceCalculationMethod, lastPriceSample`. Price fields are `Amount` (`{value: decimal-string, currency: 'USD'}`); every `*SetTime` is an RFC3339 UTC timestamp with nanosecond precision. `openInterest` is present on OPEN markets and **dropped** on expired ones; `lastPriceSample` (the only quote-derived field) is `null` on 180/180 expired markets. **This is one summary row per market, not a time series** — there is no intraday granularity, no quote history, and no depth history anywhere on the retail surface. | true | Reproduced on 270 distinct markets across 2 disjoint date windows; 5 distinct stats key-sets observed (expired vs open vs never-traded). |
| 2026-09-02 | `GET gateway /v1/markets?categories=climate` (offset walk to exhaustion) | 3683 climate markets | **The venue retains the full resolved weather universe: 3683 markets over 123 station-days, 2026-04-22 → 2026-09-03, at exactly 30 markets per station-day (5 cities × 6 rungs).** Pagination is `limit`+`offset`; `limit=500` is honoured. Resolved market objects carry `status` = `MARKET_STATUS_RESOLVED`, `closed` = `true`, `outcomePrices` = `'["0","1"]'` (terminal settlement, JSON-encoded STRING not array), and **DROP** the `bestAskQuote`/`bestBidQuote` keys that open climate markets carry. No `lastTradePrice`, no `volume`, no `liquidity`, no `priceHistory` field on the climate market object. Two station-days are ABSENT from the listing (2026-08-28, 2026-09-02) — the archive has gaps. | true | Single exhaustive walk (16 requests); date histogram reproduced on 2 independent walks (limit=100 and limit=250). |
| 2026-09-02 | `GET gateway /v1/markets/{slug}/book` — `openPx` semantics | 270 climate markets across 2 windows | **`openPx` is TRADE-derived, not a quote, and its timestamp is NOT a fixed session open.** `openPx`/`openSetTime` are absent on exactly the markets whose `sharesTraded` is `null` (52/180 in the Aug window, 53/90 in the June window; zero counter-examples in either direction), proving the field is the first PRINT, not an opening auction or a mark. `openSetTime` relative to the station-day's local-STANDARD midnight ranged **−11.0 h to +22.45 h** (median **+13.1 h**) across 128 traded markets — i.e. the modal first print lands MID-STATION-DAY, not the evening before. Only **59/128** first-printed before local midnight. A consumer must therefore treat `openPx` as 'first trade, whenever it happened', never as 'price at open'. | true | Reproduced on 2 disjoint date windows, 270 markets; the openPx↔sharesTraded equivalence held on 105 never-traded markets. |
| 2026-09-02 | `WSS /v1/ws/markets` subscribe schema (SDK 0.1.2 `websocket/types.py`) | n/a | **The markets WebSocket is forward-only; it has no replay or resume surface.** `SubscribeRequest` payload carries exactly `{requestId, subscriptionType, marketSlugs}` — no `startTime`, `since`, `resumeTime`, or snapshot-from-timestamp field of any kind (contrast the institutional gRPC balance-ledger stream, which documents `replay-from-resume_time`). Subscription types: `SUBSCRIPTION_TYPE_MARKET_DATA`, `..._MARKET_DATA_LITE`, `..._TRADE`. A `SUBSCRIPTION_TYPE_TRADE` stream exists and would capture prints going forward, but cannot be backfilled. | true | Schema read from the committed SDK; corroborated by 2 prior live WS sessions in this log that observed only `{marketData, requestId, subscriptionType}` frames. |
| 2026-09-02 | n/a | n/a | **UNRESOLVED — the request grammar of `GET /v1/price-history`.** Every GET query shape swept returns `400`/`code` 3. The most likely explanation is that the route is a gRPC-gateway transcode with a request BODY (`body: "*"`), which would make it a POST — and a POST is outside the read-only allowlist, so it was NOT attempted. Distinguishing 'GET route with an argument grammar we have not guessed' from 'POST-only route' requires either (a) a named operator exception permitting a single body-less POST probe to a NON-order path, or (b) the venue publishing the endpoint. Until then it is UNKNOWN whether this route serves price history at all, or what its response shape is. **Do not assume it is the analogue of polymarket.com's `/prices-history`; nothing about `.com` transfers.** | true | ~45 GET attempts, 1 session, 3 negative controls. |
| 2026-09-02 | n/a | n/a | **UNRESOLVED — whether the retail venue can supply a HISTORICAL ASK for a past station-day. Read-only answer today is NO.** The only retained historical price data is the per-market trade summary on `/book` (`openPx/highPx/lowPx/closePx/lastTradePx`), all trade prints; the sole quote-derived field (`lastPriceSample`) is nulled at expiry, and `bestAskQuote`/`bestBidQuote` are dropped from resolved market objects. No read-only surface exposes a resting-offer price at a past instant. Substituting `openPx` for an ask is a UNIT CHANGE, not a data source: a print is a fill, an ask is a resting offer, and the two differ by the spread and by which side initiated. | true | Reproduced on 270 markets, 2 date windows. |
| 2026-09-04 | `GET gateway /v1/markets?categories=climate` (full walk, 8 pages @ limit=500) + `/v1/series` + `/v1/events?seriesId=…` + `/v1/search` | 3743 climate markets, 5 cities | **Weather surface = 5 cities × daily HIGH only; the repo grammar covers 100% of it.** All 3743 `climate` markets parse under `_WEATHER_SLUG_RE` with city ∈ {nyc,sfo,mia,mdw,lax} (750/750/749/750/744); `question` begins 'Highest temperature in …' on 3743/3743; **zero `low` slugs**. The five `weather-daily-low-*` series (ids 36,38,40,42,44) EXIST in `/v1/series` but return **0 events** (also with `archived=true&closed=true`); series 31 `weather` → 0 events. No weather-related market exists outside `climate`: 16,165 distinct markets scanned (sports 9227, politics 2000 sampled, culture 1000 sampled, crypto 57, geopolitics 15, science 14) → 491 keyword hits, ALL sports team/mascot names (Golden Hurricane, Rainbow Warriors, Carolina Hurricanes, Miami Heat…), 0 non-sports non-climate. Station text in descriptions matches `sites.toml` exactly (KNYC Central Park, KSFO, KMIA, KMDW, KLAX); no new city. 60 open = 2026-09-04 + 2026-09-05, 30 each. | true | 1 session, 80 read-only requests, 0 errors/429/403. `politics`/`culture` sampled not exhausted; category names are a free string so an unknown category cannot be excluded. Evidence: scratchpad `evidence_out/WEATHER_SURFACE_ENUMERATION_<ts>.md` (to be moved into `docs/evidence/venue/polymarket_us/` by the coordinator — this agent may not write under `docs/`). |
| 2026-09-04 | `GET gateway /v1/markets?categories=climate` | 3743 climate markets, 2026-04-22 → 2026-09-05 | **`minimumTradeQty` DRIFTED over time on weather markets: `1` on all 1440 markets dated 2026-04-22..06-13, `0.01` on all 2303 dated 2026-06-14..09-05; no date carries both.** `orderPriceMinTickSize=0.01` and `feeCoefficient=0.06` on 3743/3743. This supersedes the 08-30 reading of a flat `0.01`: the per-market field is also per-ERA, so a consumer must read it from the live market object, never from a cached constant. | true | Single walk; the 06-14 boundary reproduced across all 5 cities (5 ladders/day). |
| 2026-09-04 | `GET gateway /v1/markets?categories=climate` | 120 markets dated 2026-04-22, 04-23, 04-29, 04-30 | **Description template drift: the earliest station-days name NO station and NO ICAO** ('recorded in New York City … as reported by the National Weather Service's Climatological Report (Daily)'), while every market dated ≥ 04-29 except those two days uses 'recorded at Central Park (KNYC) in New York City …'. 45 distinct description templates overall (5 stations × 3 comparators × 3 wordings). The station/ICAO cross-check in the parser therefore cannot be assumed present on historical markets. | true | 120/3743, 4 station-days, all 5 cities. |
| 2026-09-04 | `GET gateway /v1/markets` pagination + lifecycle | climate walk + unfiltered walk + 10 category walks | **Server page cap is 500** (`limit=1000` → 500 returned); envelope `{markets:[…]}` has no total/cursor; a short page terminates. `includeHidden=true` adds 0 climate markets (`hidden=false` on 3743/3743). **Unfiltered `/v1/markets` = 2499 markets, ALL sports, 2498 RESOLVED + 1 `MARKET_STATUS_CLOSED`** — not a venue-wide listing. Observed `category` values: climate, sports, politics, crypto, geopolitics, science, culture; `weather|economics|entertainment|esports` → 200 with 0 markets. Lifecycle: `status` ∈ {OPEN 60, RESOLVED 3683}, `closed` mirrors it, `archived=false` on all; **`active=false` occurs only on the 60 markets of 2026-04-22/23** (3623 resolved markets keep `active=true`), so `active` is not a lifecycle marker. Open-market stamps: `createdAt=startDate`=D-1 09:45Z, `gameStartTime`=D 05:00Z, `endDate`=D+1 05:00Z; event `endDate`=D 23:59Z. Archive gaps (all 5 cities): 2026-04-24..28, 06-25..27, 07-08, 07-12, 08-28, 09-02; LAX alone lacks 08-06; MIA 07-09 has a 5-rung ladder. | true | 1 session. Unfiltered 2499 vs 2599 on 08-30 — the default listing shrinks as sports resolve. |
| 2026-09-04 | `GET gateway /v1/series`, `/v1/search` | 277 series; 26 search queries | **Series objects carry `active, createdAt, id, image, recurrence, slug, subtitle, title, updatedAt` — no `closed/archived/category`; `archived=true` and `closed=true` filters returned the identical 277.** `/v1/search?query=weather&limit=50` returned exactly 10 events and `status=closed` / `status=upcoming` returned the same 10 → search result cap is 10 and `status` did not filter. `/v1/events?seriesId=35` → 124 events for 125 NYC market-days (default event listing includes closed events). | true | 1 session; series-filter inertness and search cap need a second session. |
| 2026-09-05 | `POST /v1/orders` (CreateOrderRequest) | first live order, 1 IOC BUY | **PROVISIONAL — no client-order-id field in the request body** (`submit_chain.build_order_body`); the venue cannot be queried by our `client_order_id`, and only `GET /v1/orders/open` + `GET /v1/order/{venue_id}` exist (no order history), so an AMBIGUOUS create-order with no captured venue id is resolvable only by the (market, side, qty, price, time) tuple against `/v1/portfolio/activities`, `/v1/portfolio/positions`, `/v1/account/balances` and the public book `lastTradeSetTime`. |
| 2026-09-05 | `GET /v1/portfolio/activities?marketSlug=` | 100-row page | **PROVISIONAL — the `marketSlug` filter is inert** (returns deposits and unrelated rows); filter client-side. `ACTIVITY_TYPE_TRADE` rows do carry `marketSlug`, `qty`, `price`. Same day: `/v1/account/balances` returned 500 then 503 17:46–19:51Z while public reads and the WS stayed up; a market was `MARKET_STATE_HALTED` mid station-day at 22:55Z. |
