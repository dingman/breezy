...
| 2026-09-02 | `GET api /v1/orders` | n/a (account-scoped) | **The venue does NOT implement an order-history GET: `501` / grpc `code` 12 (UNIMPLEMENTED).** This is categorically different from the `503` / `code` 14 on `/v1/orders/open`, which IS implemented and merely unavailable. Any reconciliation design that assumes a queryable order history has no endpoint to stand on. | true | Single observation. Path is present in the committed SDK snapshot, which is therefore ahead of the deployed venue. |
