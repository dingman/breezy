...
23. **Batch operations supported** (`:303–341`): batch submit via `POST /orders`, max **15** orders/request (`BATCH_ORDER_LIMIT`), LIMIT only, no retry (no idempotency key); batch cancel via `DELETE /orders`, max **1,000** IDs, chunked further by the signer's cancellation burst.
