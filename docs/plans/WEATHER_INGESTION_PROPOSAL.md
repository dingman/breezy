...
| 2 | **User-Agent: `breezy-data@gmail.com`** (role mailbox, must receive mail) | Blocks all live NWS fetching until created |
...
**Do not switch to the pyo3 catalog.** Breezy's hand-written record types (e.g. `NwsClimateDay`) carry a **Python `register_arrow` codec, not the Rust record-batch protocol** the pyo3 catalog requires. The pyo3 `CustomData.__doc__` states outright *"Custom data is always Rust-defined"*; the pyo3 catalog refuses our records at three separate points before any byte is written: (1) `register_custom_data_class(NwsClimateDay)` raises `TypeError: must have decode_record_batch_py`; (2) bare records raise `TypeError: requires CustomData wrappers`; (3) pyo3-wrapped records raise `OSError: is not registered for Arrow encoding`. Adopting the Rust codec would require `@customdataclass_pyo3`, forfeiting the strict drift-detecting decoder that §4.1 depends on (executed against 1.231.0). **A secondary, independent gate** (the directory-naming convention) prevents fallback even if the codec were adopted: pyo3 writes `data/custom/<PascalCaseClassName>/`; the Python catalog searches only `data/custom_<snake_case_class_name>/` (per `persistence/funcs.py::class_to_filename`), finds no directory, and returns **0 records silently**. This silence is critical — a half-migrated deployment produces empty backtests with no error. The pyo3 payload itself is readable; only the path differs. Since `BacktestNode.load_catalog` builds the Python catalog, both gates together mean adopting pyo3 forfeits `BacktestDataConfig` replay — contract-tested in `test_persistence_pyo3_catalog_decline.py` against the installed 1.231.0.
...
**The dedupe key cannot detect same-identity/different-bytes, so a separate index does.** Supersession keys on `(venue, city, climate_day, issuance_class, revision_seq)` and resolves by max `ts_init` (§4.3). That is correct for genuine reissues, but it means the *most* alarming case — the same NWS `product_uuid` observed twice with **different** `raw_sha256` — is silently absorbed as an ordinary revision. Upstream mutation of an already-issued product is an integrity event, not a revision. Phase 1 therefore maintains a `product_uuid → raw_sha256` **first-write-wins** index: a second observation of a known uuid whose digest differs raises a CRIT integrity alarm and hard-blocks the site, and never overwrites the first-seen digest.
...
- **Warm start reads the Actor's own catalog directly — it does NOT go through `request_data`.** Two verified framework behaviours force this, and the second is a correctness bug, not an inconvenience:
  - **`DataEngine._query_catalog` breaks on the first registered catalog that returns rows.** With one catalog root per station (above), registering all N means a warm-start query is answered by whichever root is registered first — so stations 2..N silently warm-start from **station 1's records**. That is not "warm start unavailable"; it is warm start returning confidently wrong data for four of five cities, which is exactly the class of failure this whole design exists to prevent.
  - **`catalog.custom_data(...)` drops metadata** (see below), so the response publishes on the metadata-less topic `historical.data.NwsClimateDay*`. A metadata-bearing subscription — which is what every production subscriber uses — cannot match it.
  Therefore each Actor warm-starts by reading **its own** `ParquetDataCatalog` through the existing `persistence/catalog.read_climate_days(...)`, and republishes through the same shared `DataType` factory as the live path. `request_data` and `DataEngine.register_catalog` are **not used in Phase 1**. Note `on_historical_data` is consequently never exercised; do not implement it as dead code.
...
- **`Actor.run_in_executor` cannot host the parse, and Phase 1 uses no executor at all.** `run_in_executor` returns a `TaskId` only, with no result channel reachable through the `Actor` public API. In no-executor mode, the callable's return value is discarded entirely. In executor mode, the `Future` IS retained and reachable via `ActorExecutor.get_future(task_id)`, but only if the caller holds a reference to the executor (which the `Actor` class holds privately and does not expose). For a parse that must *return* a record and be handled inline by the actor logic, `run_in_executor` is insufficient — the result is unobservable through the actor's normal interfaces.
  The requirement was itself the defect: it existed to sandbox pyIEM, and §2.5 removed pyIEM. With our own parser, a ≤128 KiB body, a structural allowlist ahead of any regex, and a wall-clock-bounded fuzz test pinning parse time, **Phase 1 parses inline on the asyncio thread and registers no executor.** The residual exposure is stated plainly rather than hidden: an inline parse stall freezes the entire Nautilus event loop, so the fuzz ceiling is a load-bearing control, not a nicety. Revisit only if a future source needs genuinely untrusted or genuinely slow parsing — and then via a path that can return a value.
- **Streaming replay (`chunk_size`) is unavailable — SETTLED by execution against the real record class.** Earlier versions could not reproduce it and marked it unverified; it reproduces. `chunk_size=2` over real `NwsClimateDay` records raises `RuntimeError: custom data type 'NwsClimateDay' is not registered with an Arrow schema containing ts_init`, at `backtest/node.py:590` → `catalog.backend_session(...)` → the Rust `DataBackendSession.add_custom_file`. **The cause is not a defect in our schema.** Streaming goes through the **Rust** backend, which cannot see a schema registered via the **Python** `register_arrow`; the one-shot path uses `_query_pyarrow` and works. The only escape is a pyo3 record type, which forfeits `BacktestDataConfig` replay entirely (§4.1) — a worse trade.
  → **Replay is one-shot (`chunk_size=None`) and therefore memory-capped.** This is a standing limit on backtest window size, pinned by `tests/contract/test_persistence_streaming_replay.py`.
- **`catalog.custom_data(cls=..., metadata=...)` silently drops `metadata`** — it is used only on the `as_nautilus=True` branch and never forwarded to `query` (`catalog/base.py:202-218`), and `as_nautilus=True` double-wraps. Only `query(metadata=...)` tags the wrapper. Another reason station separation is per-root, not per-metadata.
...
`sites.toml` (NYC) + the two record types with explicit schemas + pure `normalize/` + one `Actor` that polls via `clock.set_timer`, publishes, persists via `catalog.write_data`, and recovers via `request_data` + `on_save`/`on_load`. Includes the per-station catalog-root wiring (settled in §5: zero registered catalogs, per-Actor direct reads), the cache-database startup assertion, and re-verification of `chunk_size` against the real record class.

**Exit test:** replay the captured corpus through `BacktestNode` using one `BacktestDataConfig` per station **catalog root**, asserting the strategy's callback sequence is bit-identical to a live dry run over the same window — including a correction arriving after the initial settlement.

Two properties this test must have, because the obvious construction has neither. **It must be a divergence test, not a round-trip.** Deriving the expected sequence from the same write Run A produced only proves the catalog can read back what it wrote; it passes even if live and replay share an identical fault. Compare the **full `to_dict()`** of each record — not a hand-picked 4-tuple, which cannot see a field the projection omits — **include `ts_event`**, and pin the sequence additionally against a **hand-verified golden** fixture authored independently of Run A.
...
7. **~~Per-station catalog roots are asserted but not yet designed~~ — RESOLVED in §5.** The wiring is settled: zero catalogs registered with the `DataEngine`, each Actor reading its own root directly. Recorded here because the *reason* remains a live hazard — `_query_catalog` breaking on the first match would have made multi-station warm start silently wrong rather than merely unavailable, and nothing in the framework would have reported it.
8. **Crash between persist and publish loses the publication permanently.** The poll sequence writes, verifies, records success, then publishes; a crash in that window leaves a durably-persisted record that no subscriber ever saw, and the dedupe key suppresses any re-publish. Mitigated by making warm start the recovery path: it republishes from the catalog past the last-published `ts_init`, which must itself be part of `on_save`/`on_load` state.
...
Native facilities adopted rather than reinvented: `Data.fully_qualified_name()` for `data_cls`; `live/retry.py` backoff; `Clock.set_timer`; `Actor.on_save`/`on_load` + `Cache.add`/`get` for restart state; `BacktestDataConfig` custom-data replay; `ProbabilityPriceFeeModel`, `BinaryOption` and `pUSD` at Phase 4.

Native facilities **evaluated and deliberately declined, with the evidence**:

- `Actor.register_executor`/`run_in_executor` (no result channel reachable through `Actor` public API — cannot host a parse that must return a record, §5);
- `DataEngine.register_catalog` + `request_data` for warm start (`_query_catalog` breaks on the first registered catalog, making multi-station warm start silently wrong, §5);
- `chunk_size` streaming replay (Rust backend cannot see a Python `register_arrow` schema, §5);
- the pyo3 catalog (record types lack Rust codec protocol; directory-naming mismatch silently returns 0 records; forfeits `BacktestDataConfig` replay, §4.1);
- `instrument_id` on catalog records for multi-station separation within a single root: Nautilus `identifier_function` (parquet.py:332-333) DOES check for and use `instrument_id` if present; `_make_path` DOES append it to create paths like `/data/<type>/<id>/`; and `query(identifiers=[...])` and `BacktestDataConfig(instrument_ids=[...])` WOULD filter correctly per identifier. **However, adopting it would forfeit append-only-by-construction**: today, with identifier-less records writing flat to `data/custom_nws_climate_day/`, the substring match in `delete_data_range(identifier=None)` (parquet.py:1428) fails because the path contains no `/data/custom_nws_climate_day/` substring (the path is `/path/data/custom_nws_climate_day`, with no separator after the type name), so delete no-ops. Adding an `instrument_id` would place files at `data/custom_nws_climate_day/<id>/`, which DOES contain the substring — the condition `parts[-2] == data_cls_name` (parquet.py:1431) would pass, and delete would recursively execute. This would silently destroy the no-delete guarantee the settlement module depends on (§5.2 Corrections). Therefore, separation is achieved via **one catalog root per station** (persistent and auditable per §5, pinned by `tests/contract/test_persistence_partitioning.py`).

Declining a native facility on measured evidence is the null hypothesis being *tested*, not bypassed — none of these is reimplemented, and each is revisitable if the framework changes.
...
**Stub audit note:** Two symbols are **absent from `core/nautilus_pyo3.pyi` yet present at runtime** in 1.231.0: `nautilus_pyo3.register_custom_data_class` and `nautilus_pyo3.ParquetDataCatalog.query_custom_data`. Grepping the `.pyi` file yields false negatives; verify with `dir(nautilus_pyo3)` or by direct import. The project's configured `mypy` gate covers only `src/`, so this stub gap does not affect our code.
