...
- **`Actor.run_in_executor` cannot host the parse, and Phase 1 uses no executor at all.** `run_in_executor` **discards its callable's return value in both modes** — it returns a `TaskId` (a plain UUID with no `add_done_callback` and no result channel), and when no executor is registered it executes inline and throws the value away just the same. It is a fire-and-forget primitive. A parse that must *return* a record therefore cannot use it, and the containment story earlier versions assumed does not exist.
...
Native facilities **evaluated and deliberately declined, with the evidence**: `Actor.register_executor`/`run_in_executor` (discards return values in both modes — cannot host a parse that must return a record, §5); `DataEngine.register_catalog` + `request_data` for warm start (`_query_catalog` breaks on the first registered catalog, making multi-station warm start silently wrong, §5); `chunk_size` streaming replay (Rust backend cannot see a Python `register_arrow` schema, §5); the pyo3 catalog (on-disk layout incompatible; forfeits `BacktestDataConfig` replay, §4.1). Declining a native facility on measured evidence is the null hypothesis being *tested*, not bypassed — none of these is reimplemented, and each is revisitable if the framework changes.
