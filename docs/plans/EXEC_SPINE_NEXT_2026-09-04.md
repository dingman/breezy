...
| R-8 | first real order: 1 contract, marketable, IOC; store `avgPx`+`cumQuantity` | — | parent §R-8 | R-7, R-8-PRE-2 | YES: `BREEZY_TRADING_ENABLED=1`, `BREEZY_MAX_DAILY_BUDGET_USD`, `BREEZY_MAX_POSITION_COST_USD` in the operator's shell only | goal-state clauses 1–6 |
