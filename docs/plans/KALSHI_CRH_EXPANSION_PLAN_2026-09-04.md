...
- `settlement_delay_time_local` / `settlement_delay_timezone` — **UNVERIFIABLE from S0.** Kalshi publishes no CLI-disagreement review delay; its nearest mechanism is the 300 s `determined` dispute window, which is a different thing. Pinned pessimistically at `"08:00"`/`"America/New_York"` (i.e. no earlier release than the deadline) and **flagged in the registry comment as venue-unconfirmed**, to be replaced by the 20-settlement observation campaign's measurement in S15.
...
|**S4**|Registry `[sites.kalshi.{LAX,MDW,MIA,SFO}]` incl. the five settlement-clock fields (08:00 / America/New_York / 08:00 / America/New_York / 7), delay pair commented venue-unconfirmed|`sites.toml:71-75,94-104`; `sites.py:62-68`|`BREEZY_SITES=kalshi:LAX` validates; body-header regex asserted against a live product|
