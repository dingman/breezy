...
- **PREREG v2 REGISTERED 2026-09-05** (`4975fba`; spec BINDING §13, manifest `pm_us_crh_v2` REGISTERED d0=2026-09-05, `breezy-pm-crh-v2-tally.timer` 15:30 UTC enabled, first run n=0). Open: strategy-lead acknowledgement of C1–C4 (`grok_registration_memo` in flight); Kalshi sibling stays DRAFT.
...
- **[MED] Daily relaunch supervisor LIVE** (`breezy-trade-supervisor`, `cbcb1ab..e335b80`, pid own-SID since 09-05 00:25Z; peer-reviewed, plan Rev 3). Open: it configures no log handler (decision lines dropped) — fix in flight, then restart before 16:40 UTC; first unattended cycle 09-05 16:40/16:50/17:05 UTC must be verified (`~/.local/share/breezy/logs/breezy-trade-supervisor.log`). Fallback: session cron 16:20 UTC relaunches by hand (L-26).
- **[HIGH] Live fill → scored-trial chain is UNSCHEDULED.** `score_live_trials.py` is a placeholder JSONL driver invoked by no unit; both tallies (14:30/15:30 UTC) read a store nothing writes on the live path. A real fill today would never be scored. Mapping in flight (code-explorer), then planning gate + build before the first fill.
