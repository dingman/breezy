...
- **[MED] PREREG v2 family** (`docs/specs/PREREG_v2_current_rung_hold_DRAFT_2026-09-04.md` rev b; blueprint `docs/plans/FAMILY_TALLY_V2_BLUEPRINT_2026-09-04.md` Rev 2; items 1,3–6,8 landed). LANDED 09-04: generator `9e0ad5a`/`0e264fa` (terminal look spends the remaining α; cumulative 0.025; domain-reviewed), loader + `family_tally_v2.py` `78e37ff` (SHADOW-only while DRAFT), fill admission excludes q≠1 `c7240a8` (ruling `grok_partial_fill_ruling_2026-09-04.md`). Open: domain + code review of `78e37ff`, then REGISTRATION before the first PM fill — spec v2 → BINDING, manifest `status`/`d0_climate_day`, enable `breezy-pm-crh-v2-tally.timer` (15:30 UTC). Codex pipeline review dispositions: `docs/evidence/codex_measurement_pipeline_review_2026-09-04.md`.
...
- **[HIGH] No session-independent daily relaunch.** Plan `docs/plans/TRADE_NODE_DAILY_RELAUNCH_2026-09-04.md` (Rev 3 in convergence: security APPROVE, architecture REVISE). Prerequisite LANDED `580b56c` (permit-issued INFO line + `LIVE_TRADING_PERMIT_REFUSED` WARN alert); supervisor `runtime/trade_supervisor.py` build in flight (values-free module, 16:40 stop / 16:50 launch / 17:05 self-check). Until it runs: every coordinator session checks `pgrep -f 'breezy-trade$'` at start and relaunches before 17:00 UTC (L-26).
